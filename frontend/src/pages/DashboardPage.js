...
import { BRAND_NAME, PAGE_TITLES, DEMO_MODE, LANDING_URL, LOGO_URL } from '@/constants';
...
// ── DEMO DATA ──
const DEMO_STATS = {
  cargas_disponibles: 47,
  envios_en_curso: 12,
  completados_mes: 156,
  transportistas_activos: 89
};
...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      // Si DEMO_MODE está activo, usar datos de demo directamente sin llamar a la API
      if (DEMO_MODE) {
        setStats(DEMO_STATS);
      ...
      try {
        ...
        if (res.ok) { setStats(await res.json()); } else throw new Error();
      } catch { setStats(DEMO_STATS); }

      try {
        const res = await fetch(`${API_URL}/api/cargas`);
        if (res.ok) { setCargas(await res.json()); } else throw new Error();
      } catch { setCargas(DEMO_CARGAS); }

      try {
        const res = await fetch(`${API_URL}/api/envios`);
        if (res.ok) { setEnvios(await res.json()); } else throw new Error();
      } catch { setEnvios(DEMO_ENVIOS); }

      try {
        const res = await fetch(`${API_URL}/api/transportistas`);
        if (res.ok) { setTransportistas(await res.json()); } else throw new Error();
      } catch { setTransportistas(DEMO_TRANSPORTISTAS); }
    };
  ...
  const sendChatMessage = async (message) => {
    ...
    // Modo DEMO: respuesta simulada sin llamar a la API
    if (DEMO_MODE) {
      setTimeout(() => {
        const lowerMsg = msg.toLowerCase();
        let response = 'Estoy en modo demostración. Puedo ayudarte a explorar las cargas disponibles en el panel.';
        let cargasEncontradas = [];

        if (lowerMsg.includes('madrid') || lowerMsg.includes('barcelona')) {
          cargasEncontradas = DEMO_CARGAS.filter(c => 
            c.origen.toLowerCase().includes('madrid') || 
            c.destino.toLowerCase().includes('barcelona') ||
            c.origen.toLowerCase().includes('barcelona') || 
            c.destino.toLowerCase().includes('madrid')
          );
          ...
          cargasEncontradas = DEMO_CARGAS.filter(c => 
            c.origen.toLowerCase().includes('valencia') || 
            c.destino.toLowerCase().includes('valencia')
          );
          response = `He encontrado ${cargasEncontradas.length} cargas relacionadas con Valencia:`;
        } else if (lowerMsg.includes('urgente')) {
          cargasEncontradas = DEMO_CARGAS.filter(c => c.tipo === 'urgente');
          response = `Hay ${cargasEncontradas.length} cargas urgentes disponibles:`;
        } else if (lowerMsg.includes('hola') || lowerMsg.includes('ayuda')) {
          response = '¡Hola! Estoy en modo demostración. Puedes preguntarme sobre cargas por ciudad (Madrid, Barcelona, Valencia) o tipo (urgente, frigorífico, etc.).';
        }

        const assistantMsg = {
          role: 'assistant',
          content: response,
          cargas: cargasEncontradas.length > 0 ? cargasEncontradas : undefined,
    ...
    <aside className={`sidebar ${mobile ? 'mobile' : ''} ${collapsed ? 'collapsed' : ''}`} onClick={e => e.stopPropagation()}>
      ...
      {collapsed && (
        <div className="sidebar-footer-collapsed">
          {DEMO_MODE && <span className="demo-dot" title="Modo Demo"></span>}
          <img
            src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop"
            alt="Avatar"
